...
def analyze_video_frames(
    ...
    full_prompt = (
        ...
        f"问题：{question}\n\n"
        f"强制要求：\n"
        f"1. 只提取视频中明确出现的信息，绝对不要编造、推断或补全。\n"
        f"2. 优先提取可用于后续结构化查询的条件：阈值、年份、日期、代码、名称、筛选条件、排序口径、字段名。\n"
        f"3. 如果视频展示的是筛选配置、查询条件、表头、口径说明，就输出这些条件本身，而不是擅自计算最终表格答案。\n"
        f"4. 注意视频中所有文字：包括字幕、屏幕显示、表格、名片、图表等。\n"
        f"5. 所有数值、姓名、电话号码、日期必须精确到字符级，保持原格式。\n"
        f"6. 如果视频中完全没有找到相关信息，直接回答 unknown。\n"
        f"7. 不要输出思考过程，不要加解释性前后缀。"
    )
